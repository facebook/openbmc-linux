...
/* If both nibbles are in the [0xA, 0xF] range, the returned value will be
 * zero. This means that optional fields (encoded as 0xFF) will default to
 * zero, which should be fine.
 */
...
/* Response handler for Meta card */
static int ncsi_rsp_handler_oem_meta(struct ncsi_request *nr)
{
	return 0;
}
...
static struct ncsi_rsp_oem_handler {
	...
	{ NCSI_OEM_MFR_META_ID, ncsi_rsp_handler_oem_meta }
};
...
static int ncsi_rsp_handler_gmcma(struct ncsi_request *nr)
{
	...
	for (i = 0; i < rsp->address_count; i++) {
		...
		netdev_warn(ndev, "NCSI: Success using MAC address %d\n", i);
		break;
	}
