/*
 *  ast_sdhci.c - ASPEED Secure Digital Host Controller Interface driver
 *
...
static int sdhci_resume_host(struct sdhci_host *host)
{
	int ret;
/*
	if (host->vmmc) {
...
#else
#define sdhci_suspend_host NULL
#define sdhci_resume_host NULL
...
#define	SDIO_ALL_SOFTWARE_RESET		0x01
	...
	*(unsigned int*)(IO_ADDRESS(SDIO004)) = 0x1000;
	...
	//Issue : DMA con-current with UHCI dma, so force to PIO mode
