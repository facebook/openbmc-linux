				      ...
				      u32 ibi_status)
{
	...
	if (!dev) {
		dev_err(master->dev, "no matching dev for addr 0x%x, ibi status 0x%x\n",
			addr, ibi_status);
		goto out;
	}
...
static void aspeed_i3c_master_demux_ibis(struct aspeed_i3c_master *master)
{
	...
	for (i = 0; i < nibi; i++) {
		...
		if (IBI_TYPE_MR(status))
			dev_info(master->dev,"get mr from %02x\n", addr);
	}
}
			  ...
			  struct i3c_ccc_cmd *ccc)
{
	...
	ret = xfer->ret;
	if (ret && !IS_RESPONSE_ERROR_NACK(xfer->cmds[0].error))
				      ...
				      struct i3c_slave_payload *payload)
{
	...
	if (SLV_INTR_REQ_IBI_STS(intr_req) != SLV_IBI_STS_OK) {
		slv_event = readl(master->regs + SLV_EVENT_CTRL);
		if ((slv_event & SLV_EVENT_CTRL_SIR_EN) == 0)
			dev_err(master->dev, "sir is disabled by master\n");
		return -EACCES;
	}
