...
static ssize_t broadcast_hj_enable_store(struct device *dev, struct device_attribute *da,
                               const char *buf, size_t count)
{
    struct i3c_master_controller *master;
    ssize_t ret = count;
    ...
    master = dev_to_i3cmaster(dev);
    dev_dbg(&master->dev, "send HJ enabling\n");
    ret = i3c_master_enable_hj(master);
    return ret ? 0 : count;
}
static DEVICE_ATTR_WO(broadcast_hj_enable);
...
static struct attribute *i3c_masterdev_attrs[] = {
	...
	&dev_attr_broadcast_hj_enable.attr,
	NULL,
};
	...
	if (olddev) {
		...
		if (olddev->ibi) {
			...
			if (olddev->ibi->enabled) {
				enable_ibi = true;
				if (i3c_dev_disable_ibi_locked(olddev))
...
int i3c_dev_send_ccc_cmd_locked(struct i3c_dev_desc *dev, u8 ccc_id)
{
	...
	case I3C_CCC_DISEC(false):
		ret = i3c_master_disec_locked(master, dev->info.dyn_addr, I3C_CCC_EVENT_SIR | I3C_CCC_EVENT_MR | I3C_CCC_EVENT_HJ);
		break;
	case I3C_CCC_ENEC(false):
		ret = i3c_master_enec_locked(master, dev->info.dyn_addr, I3C_CCC_EVENT_SIR);
		break;
	default:
