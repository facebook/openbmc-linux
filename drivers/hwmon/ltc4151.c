...
static int ltc4151_detect(struct i2c_client *client, int kind,
		       struct i2c_board_info *info) {

	struct i2c_adapter *adapter = client->adapter;
  dev_info(&client->dev, "detect called for '%s'", client->name);
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
    dev_info(&client->dev, "detect: i2c_check_functionality failed");
		return -ENODEV;
  }
  dev_info(&client->dev, "cur type: %s", info->type);
  return -ENODEV;
}
			 ...
			 const struct i2c_device_id *id)
{
	...
	int ret;
  dev_info(&client->dev, "probin '%s'", client->name);
    ...
    dev_info(&client->dev, "hwmon register failed");
