...
void ast_uhci_reset_hc(struct uhci_hcd *uhci);
int ast_uhci_check_and_reset_hc(struct uhci_hcd *uhci);
...
//	if (uhci_check_and_reset_hc(to_pci_dev(uhci_dev(uhci)), uhci->io_addr))
	if (ast_uhci_check_and_reset_hc(uhci))
		finish_reset(uhci);
}
...
/*-------------------------------------------------------------------------*/
static int usb_hcd_ast_probe (const struct hc_driver *driver,
...
static inline void
usb_hcd_ast_remove (struct usb_hcd *hcd, struct platform_device *pdev)
{
	struct uhci_hcd *uhci = hcd_to_uhci(hcd);
...
/*-------------------------------------------------------------------------*/
#ifdef	CONFIG_PM
static int uhci_ast_suspend(struct platform_device *dev, pm_message_t message)
{
...
static int uhci_ast_resume(struct platform_device *dev)
{
...
static int uhci_hcd_ast_drv_probe(struct platform_device *dev)
{
	return usb_hcd_ast_probe(&_uhci_driver, dev);
}
static int uhci_hcd_ast_drv_remove(struct platform_device *dev)
{
	struct usb_hcd		*hcd = platform_get_drvdata(dev);

	usb_hcd_ast_remove(hcd, dev);
 ...
 */
static struct platform_driver uhci_hcd_ast_driver = {
	.probe		= uhci_hcd_ast_drv_probe,
	.remove		= uhci_hcd_ast_drv_remove,
#ifdef	CONFIG_PM
	.suspend	= uhci_ast_suspend,
	.resume		= uhci_ast_resume,
#endif
	.driver		= {
		.owner	= THIS_MODULE,
		.name	= "ast_uhci",
...
static int __init uhci_hcd_ast_init (void)
{
	uhci_hcd_init();
	return platform_driver_register(&uhci_hcd_ast_driver);
}

static void __exit uhci_hcd_ast_cleanup (void)
{
	uhci_hcd_cleanup();
	platform_driver_unregister(&uhci_hcd_ast_driver);
}

//module_init(uhci_hcd_ast_init);
//Ryan modify for message UHCI must after EHCI
late_initcall(uhci_hcd_ast_init);
module_exit(uhci_hcd_ast_cleanup);
