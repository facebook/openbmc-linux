...
static u32 ast2600_select_i2c_clock(struct ast2600_i2c_bus *i2c_bus)
{
	...
	if (i2c_bus->clk_div_mode) {
		...
		/* Customize clock  */
		if (!of_property_read_u32(pdev->dev.of_node, "base-clock-divisor", &tmp))
			baseclk_idx = (tmp & 0xF);

		if (!of_property_read_u32(pdev->dev.of_node, "tck-high", &tmp))
			scl_high = (tmp & 0xF);

		if (!of_property_read_u32(pdev->dev.of_node, "tck-low", &tmp))
			scl_low = (tmp & 0xF);
...
static int ast2600_i2c_master_irq(struct ast2600_i2c_bus *i2c_bus)
{
	...
	if(!i2c_bus->msgs) {
		dev_dbg(i2c_bus->dev, "null i2c message\n");
		writel(AST2600_I2CM_PKT_DONE, i2c_bus->reg_base + AST2600_I2CM_ISR);
		complete(&i2c_bus->cmd_complete);
		return 1;
	}
